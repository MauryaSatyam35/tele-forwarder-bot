...
async def process_outbox(context: ContextTypes.DEFAULT_TYPE):
    """Process pending outbox entries once. Intended to be scheduled with JobQueue.run_repeating."""
...
def main():
    ...
    # Startup helper: schedule process_outbox either on JobQueue (if available)
    # or start a background runner after application initialization.
    async def _start_background(application: Application):
        try:
            if getattr(application, 'job_queue', None):
                # JobQueue is available (installed extras); use it
                application.job_queue.run_repeating(process_outbox, interval=1, first=1)
                logger.info('Scheduled process_outbox on JobQueue')
            else:
                # Fallback: start a background runner in the app's event loop
                async def _runner():
                    while True:
                        try:
                            ctx = SimpleNamespace(application=application)
                            await process_outbox(ctx)
                        except Exception as e:
                            # avoid noisy crashes
                            logger.exception('Background outbox runner error: %s', e)
                        await asyncio.sleep(1)

                application.create_task(_runner())
                logger.info('Started background outbox runner (no JobQueue)')
        except Exception:
            logger.exception('Failed to start background tasks')

    # Build application with PTB v20.5 API and register post-init hook
    builder = Application.builder().token(BOT_TOKEN).concurrent_updates(True)
    # Schedule background tasks after the application has fully started to avoid
    # PTB warnings about creating tasks before the app is running.
    # Use post_startup (preferred) if available; fall back to post_init.
    try:
        builder.post_startup(_start_background)
    except Exception:
        builder.post_init(_start_background)
    app = builder.build()
    ...
    # outbox processing will be scheduled by the post_init hook (_start_background)
    # which will use JobQueue if available or start a fallback background runner.

    # Start the bot (blocks until stopped)
    try:
        stats['status'] = 'Initializing...'
        console.print("[bold green]Starting application (polling)...[/bold green]\n")
        app.run_polling()
